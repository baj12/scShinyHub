...
```{r featureDataReact, include=FALSE}
...
```{r pca, eval=TRUE, include=FALSE}
if(DEBUG)cat(file=stderr(), "pca\n")

pca = pcaFunc(gbm_log)
```

```{r kmClustering, eval=TRUE, include=FALSE}
...
```{r tsne, eval=TRUE, include=FALSE}
...
```{r prioritized_genes, eval=FALSE, include=FALSE}
set.seed(seed = seed)
