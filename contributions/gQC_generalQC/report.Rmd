...
```{r bj-gQC-setup, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, include=FALSE}
if(exists("params")){
...
cat(file=stderr(),paste("bj-gQC-tsne3d: ", class(tsneData), "\n"))
...
```{r bj-gQC-tsne3d, eval=FALSE, echo=FALSE}
...
```{r gQC_plotUmiHist, eval=TRUE, echo=FALSE}

hist(Matrix::colSums(assays(scEx)[["counts"]]), breaks = 50, main="histogram of number of UMI per cell")
```

## Variances of PCs

```{r gQC_variancePCA, eval=TRUE, echo=FALSE}
...
```{r histSamples, eval=TRUE, echo=FALSE}
if(DEBUGSAVE) 
...
```{r savegqcProj, echo=FALSE}

# save full table in tmp folder to be included in report
  write.csv(projections, file = paste0(input$reportTempDir, "/gqcProjections.csv"))
