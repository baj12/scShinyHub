...
require(pheatmap)
saveFile = "~/scShinyHubDebug/coE_heatmapFunc.report.RData"
save(file = saveFile, list = c(ls(), ls(envir = globalenv())))
# load(file = saveFile)
...
```{r selectd2D, echo=FALSE, eval=TRUE}
#TODO uses module and thus not working correctly if not executed
# g_id <- input$gene_id
...
ccols <- clusterCols$colPal
save("~/Rstudio/shHubgit/tmp.RData")
# genesin <- input$coE_heatmap_geneids2
# input$`coE_selected-clusters-selectized`
...
#   }
# 
# plot2Dprojection(scEx_log, projections, g_id, featureData, geneNames, geneNames2, dimX, dimY, clId, grpN, legend.position, 
#                  grpNs = groupNames$namesDF, colors = myColors)
...
write.csv(outMat, file = paste0(reportTempDir, "/output_coE_topExpGenes.csv"))
